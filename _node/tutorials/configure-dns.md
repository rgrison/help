...
Cloud 66 provides [DNS hostnames](/node/references/server-ip-addresses.html) for each server you deploy with us. This allows us to assign a new IP address to your application on your behalf if need be, while still maintaining the same hostname.
...
3.  ​Use [network redirects](/node/tutorials/service-network-configuration.html) to permanently redirect all traffic from example.com to www.example.com.
